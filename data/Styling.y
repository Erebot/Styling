...
%syntax_error { throw new \InvalidArgumentException(); }
%token_prefix TK_
%include {
...
%include_class {
    ...
    private function _extractNumber($var)
    {
        ...
        if ($var instanceof \Erebot\Styling\Variables\IntegerInterface)
            return $var->getValue();

        if ($var instanceof \Erebot\Styling\Variables\FloatInterface)
            return $var->getValue();
...
expr(res) ::= VARIABLE(opd).                {
    if (!isset($this->_tplVariables[opd]))
        throw new \InvalidArgumentException('No such variable: '.opd);
